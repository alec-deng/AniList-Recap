...
- `apollo/client.ts` — popup's own Apollo client; auth header read fresh from `chrome.storage.local` per request (not React state). An `onError` link watches for AniList's `400 "Invalid token"` and sends `SESSION_EXPIRED` to the background, which clears the session so the popup falls back to `LoginPage`. It only observes — the error still reaches `useQuery`, so each tab's `StateMessage` renders as before. Guarded by a module flag because a popup open fires several queries at once. Nothing queries while logged out (every query is `skip: !userId`, and the id comes from storage), so the strict `400` + `"Invalid token"` test has no logged-out traffic to misfire on; a dead token still trips it through the settings/list queries.
- `hooks/useUserId.ts` — `userId` from the `user` the background stored at login, **never a `Viewer` query**. Every request needing the id (settings, both list tabs, stats) is `skip: !userId`, so querying it puts one serial round trip in front of all of them — on a cold open that was the whole first wave, and AniList is the slow part. Re-reads on `storage.onChanged`, since the providers outlive a logout.
- `contexts/SettingsContext.tsx` — AniList-hosted prefs (profile color, title language, adult flag, score format, row order) + local-only settings (`manualCompletion`, `separateEntries`, `tabVisibility`, `showAnimeStats`/`showMangaStats`). Changing `tabVisibility` resets both stats toggles to match, but they stay independently overridable after — don't re-couple. Surfaces `error` alongside `loading`; every tab must check its own `error` or a failed query silently degrades to "0 items." The five AniList prefs are also mirrored to `chrome.storage.local` (on query result *and* in their setters) purely as a paint cache — the popup opened on `blue` and repainted a round trip later otherwise. `serverPrefsRef` keeps the query's answer winning over the cache whichever lands first, and the cache is stamped with `prefsUserId` so it's only seeded for the account it came from — that, not clearing on logout, is what keeps a same-account re-login fast while never showing another user's score format. The local-only settings are device preferences and load unstamped, logged in or out.
...
- `components/MediaListTab.tsx` — the whole list-tab pipeline (query → transform → adult filter → sort → caught-up split → grid) plus optimistic update → `QUEUE_UPDATE` → dirty-flag, `useStableOrder`, and the **reorder/removal choreography**. `AnimeTab.tsx`/`MangaTab.tsx` are thin `MediaListConfig` wrappers — put list-behavior fixes in `MediaListTab`, not the wrappers. Renders through `MediaCard` (generic over anime episodes / manga chapters).
  ...
  - **The refreeze must land in a later commit than the release** (`scheduleRefreeze` + its `useLayoutEffect`). `useStableOrder` re-reads the live order *during render*, so a render with `holdOrder === false` is the only thing that reorders the grid; batch release and refreeze together and the card plays its whole drop-in (`.card-entering`/`.card-settling`) in its **old** slot, then jumps unanimated when the freeze is next released. Don't go back to `requestAnimationFrame` — a frame can be served before React's scheduler task, and a long list widens the window since the release render is interruptible. Keep the effect keyed on `holdOrder`: dep-array-free, it runs after every commit and blocks paint on each one during load.
