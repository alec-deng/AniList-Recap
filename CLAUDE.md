...
- **Hover reaches this system through two channels that can disagree.** The arrows are CSS (`group-hover:`); the freeze is React's `onMouseEnter`, i.e. a native `mouseover`. Two things stop that ever firing on a long list, where the grid paints well after the pointer has settled: content inserted under a *resting* pointer gets `:hover` recomputed but no `mouseover`, and moving *within* the card afterwards only produces a `mouseover` whose `relatedTarget` is already inside the tree, which React discards by design. So a card could stay un-entered for its whole life — arrows working, `hoveredId` null, every edit re-sorting live. `MediaCard` therefore also seeds hover from **`onMouseMove`**, the one signal both cases share; enter stays as the fast path. **Don't drop it as redundant**, and don't reach for `matches(":hover")` instead (tried — the browser's hover pass can land after the mount effect). Generally: assume no boundary event for any DOM change under a still pointer.
...
- `MediaCard`'s arrow clusters and "Mark as Completed" reveal on `group-hover` **or `group-[:has(:focus-visible)]`, not `group-focus-within`** — a mouse-clicked button keeps DOM focus, so `focus-within` left the arrows lit after the pointer had gone. A click doesn't match `:focus-visible`, a Tab does, and a focused text input always does, so the mid-edit reveal is unchanged.
